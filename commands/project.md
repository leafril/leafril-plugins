# Project

기능 개발 문서(PRD + 일정 + 진행 상황)를 생성·수정하는 서브커맨드입니다.
문서 구조·메타데이터 규칙은 노트 루트의 CLAUDE.md에 위임한다.

## 실행 프로토콜

### 전제조건: 노트 루트 경로 확인

`~/.claude/note-plugins.json`에서 `noteRoot` 값을 확인한다. 파일이 없으면 새로 생성한다. `noteRoot`가 없거나 존재하지 않는 경로이면 AskUserQuestion으로 물어보고, 응답을 저장한다.

### Phase 1: 대상 파일 결정

1. 인자로 `{프로젝트}/{기능명}` 또는 `{기능명}`이 전달되면 해당 파일을 찾거나 생성
2. 인자 없이 호출되면 노트 루트의 `projects/` 하위 파일 목록을 보여주고 AskUserQuestion으로 선택
3. 프로젝트명이 생략되면 현재 작업 디렉토리의 프로젝트명을 추론

### Phase 2: 파일 읽기

- 기존 파일이 있으면 읽고 내용을 파악
- 없으면 새 파일 생성 준비

### Phase 3: 작성·수정

사용자가 전달한 내용에 따라 문서를 생성하거나 수정한다.

**daybook 링크 추가:** 사용자가 daybook 링크 추가를 요청하면, `## Daybook` 섹션에 `- [[{DD} {Day}/{주제}]]` 형식으로 링크를 추가한다 (중복 방지).

### Phase 4: Git 커밋 (선택)

작성 완료 후 커밋 여부 확인.

## 주의

- 구조화된 질문으로 사용자를 유도하지 않는다.
- 사용자가 말한 내용을 그대로 반영한다. 임의로 작업을 추가하거나 일정을 조정하지 않는다.
