...
config 파일 내부에 `cache` 섹션을 두어 스키마·테이블 메타데이터를 저장한다. 매 쿼리마다 `information_schema` 조회를 생략하여 응답 속도를 높이고, 테이블 설명으로 자연어 매칭 정확도를 개선한다.
...
- 캐시 생성: 최초 스키마/테이블 조회 시 자동 생성
- 캐시 갱신: `/sql refresh` 명령어로 명시적 갱신
- 설명 업데이트: 쿼리 과정에서 사용자 피드백이 있으면 자동 반영 (§7 참조)
...
1. **`refresh`로 시작하면** → 캐시 갱신 흐름 (§7) 실행. `refresh` 뒤에 별칭이 있으면 해당 DB만, 없으면 전체 갱신
...
**캐시 미스 시** 메타쿼리로 조회한 후 결과를 캐시에 저장한다:
...
| 조건 | 동작 |
|------|------|
| 캐시에 해당 스키마의 `tables` 존재 | 캐시에서 테이블 목록 + 설명 로드 |
| 캐시 미스 | 메타쿼리로 조회 → 캐시에 저장 (설명은 빈 문자열) |
...
- 사용자가 선택하면 **캐시의 테이블 설명을 자동 업데이트**한다 (§7 참조)
...
쿼리 과정에서 사용자가 테이블에 대한 도메인 지식을 언급하면 캐시 설명을 자동 업데이트한다:
...
테이블 설명이 변경되면 config 파일의 `cache` 섹션을 즉시 Write한다.
...
자연어 요청 도착
  ↓
캐시에 스키마 목록 있음? ─ No ──→ DB 조회 → 캐시 저장
  │ Yes
  ↓
캐시에 테이블 목록 있음? ─ No ──→ DB 조회 → 캐시 저장
  │ Yes
  ↓
테이블 설명으로 매칭 시도
  ↓
매칭 후보 1개? ─────────→ 바로 사용
  │ 2개 이상
  ↓
AskUserQuestion으로 질문 → 사용자 선택 → 캐시 설명 업데이트 → 사용
```
