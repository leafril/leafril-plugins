...
경로: `{프로젝트 루트}/.claude/sql-config.json`

비밀번호가 포함되므로 반드시 `.gitignore`에 등록해야 한다. 등록되어 있지 않으면 config 생성 시 자동으로 추가한다.
...
`{프로젝트 루트}/.claude/sql-config.json`을 읽는다.

- **파일 있음** → 접속 정보 로드
- **파일 없음** → 사용자에게 접속 정보를 질문하고 파일 생성. `.gitignore` 등록 확인

### Step 2: 요청 파싱 및 쿼리 준비

| 입력 유형 | 처리 |
|-----------|------|
| SQL 직접 제공 | §1 허용 규칙 확인 → 확인 생략 → Step 4로 |
| 자연어 요청 | 테이블 매칭 → SELECT 쿼리 생성 → 사용자에게 보여줌 → 확인 대기 |

#### 자연어 → 테이블 매칭

자연어 요청 시 테이블명을 모르면 메타쿼리로 테이블 목록을 조회한다:
...
- **DB**: 별칭 지정 없음 → config의 `default` DB 사용. 별칭 지정 있음 → 해당 DB 사용
- **스키마**: `--schema` 지정 없음 → config의 `default_schema` 사용. 지정 있음 → 해당 스키마 사용. 쿼리 내 `schema.table` 형태로 명시된 경우 그대로 사용
