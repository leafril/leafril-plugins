...
4. `tasks[].criteria`와 `invariants`를 모두 훑어 `verify: "test"` criterion이 하나라도 있으면 **두 파일을 모두 읽는다**:
...
```
1. 코드 작성
2. 테스트 작성 (해당 task에 behavioral criterion — `verify: "test"` 또는 `playwright:*` — 이 있으면 필수)
...
**`verify: "bash:*"` 전용 criterion만 있는 task**: 테스트 작성이 필요 없는 경우도 있다 (예: `bash:npm run build`가 유일한 criterion). 이때는 2번을 건너뛰고 3번에서 해당 bash 명령을 실행한 뒤 결과로 판정한다.

**이 단계에서 하지 않는 것**:
- Evaluator 호출 (Step 5에서 명시 요청 또는 feature 완료 시점에만)
- `criteria.status` / `evidence` 갱신 (evaluator만 건드림)
...
- **테스트 작성 없이 behavioral criterion task 넘기기**. `verify: "test"`/`playwright:*`이 있으면 반드시 테스트 작성 후 통과 확인
