...
- 예시:
  - 신기능: "사용자가 소셜 계정으로 로그인할 수 있다"
  - 리팩터: "게임 루프 책임 분리로 신규 기능 추가 비용을 낮춘다"
  - 성능: "목록 화면 초기 로드를 체감 즉시 수준으로 단축한다"

### features 작성 규칙

- **외부 관측 가능한 동작·지표·불변식**으로 적는다. 모듈명·클래스명·라이브러리 단위 금지
  - ❌ "HUD Scene 분리", "Score 클래스 추출"
  - ✅ 신기능: "점수와 콤보가 게임 진행에 따라 화면에 반영된다"
  - ✅ 리팩터: "기존 점수·콤보 계산 결과가 리팩터 전후 동일하다" (회귀 불변식)
  - ✅ 성능: "목록 화면 초기 로드가 3s → 1s 이내", "동시 500명에서 p95 200ms 유지"
- **단독 머지 가능 단위**로 쪼갠다. 중간 단계만 머지해도 의미가 있어야 함
- **배열 순서 = 구현 순서**. 앞 항목이 뒷 항목의 전제가 되도록 쓴다 (중간에 멈춰도 깨지면 안 됨)
...
- "성능 개선"인데 측정 가능한 before→after 지표 없음 (❌ "쿼리 최적화" → ✅ "p95 500ms → 150ms")
