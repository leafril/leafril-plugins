...
description: >
  ...
  rules 파일의 체크리스트(§7)로 점검. evaluator coordinator가 testCriteria 검증 시 호출.
model: sonnet
...
```json
[
  { "criterion": "동기화 ��직이 올바르게 작동한다" },
  { "criterion": "할인율 10%일 때 1000원 상품이 900원으로 계산된다" }
]
```
...
```
RESULTS:
- criterion: "동기화 로직이 올바르게 작동한다" | result: PASS | evidence: "5개 테스트 점검 완료: 이름 common §6 준수, assert common §2 준수, mock common §5 준수, 중복 없음, 데이터 충분"
- criterion: "할인율 10%일 때 1000원 상품이 900원으로 계산된다" | result: FAIL | evidence: "common §7-1 위반: 테스트명 'calculateDiscount - 할인��� 계산한다'에 메서드명 포함. common §7-3 + §K4-3 위반: OrderRepository(managed)에 verify() 사용"
```
