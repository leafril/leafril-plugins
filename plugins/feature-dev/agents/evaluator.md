...
evaluator-playwright 에이전트의 절차에 따라 검증하고 RESULTS 형식으로 출력하라.
```
...
| Verdict | 조건 |
|---------|------|
| **PASS** | criteria 전체 PASS + 컨벤션 위반 0건 |
| **FAIL** | 그 외 |

## 원칙

- 구현 코드를 수정하지 않는다. **Why**: evaluator가 코드를 고치면 검증 독립성이 훼손됨
...
| 컨벤션 위반이 경미함 (주석 오타, trailing space 등) | 위반으로 기록하되 Recommendations로 분류. Verdict 판정에는 불포함 |
